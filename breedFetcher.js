...
const findKeyByValue = function(object, value) {
  for (let item in object) {
    if (value === object[item]["name"]) {
      return JSON.stringify(object[item]["description"]);
    }
  }
  return `Oops - Looks like ${value} is not found`;
};
//----------------------------------------------------------
...
const fetch = (url, path, breed) => {
  request(url, (error, response, body) => {
    ...
    // Error Free - continue with code
      ...
      const breedData = findKeyByValue(data, breed);
      fs.writeFile(path, breedData, err => {
        if (err) {
          console.error(err);
        } else {
          ...
          fs.stat(path, (err, stats) => {
            if (err) {
              ...
              console.log(`${conColor.cyan}${breedData}${conColor.reset}`);
            }
          });
        }
      });
...
const helper = (check, breed) => {
  if (!check) {
    ...
    fetch("https://api.thecatapi.com/v1/breeds", process.argv[2], breed);
    rl.close();
  } else {
